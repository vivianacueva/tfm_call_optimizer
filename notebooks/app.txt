...
# Streamlit UI
st.set_page_config(page_title="Asistente GenAI Call Center", layout="wide")
st.title("Asistente GenAI Call Center")
...
user_input = st.chat_input("Haz una pregunta sobre esta llamada o sobre los procedimientos generales de resolución...")

if user_input:
    response = qa_chain.invoke({"query": user_input})
    answer_raw = response["result"]

    # 🧠 Aplicar regex para limpiar el texto y quedarnos solo con lo útil
    match = re.search(r'Useful Answer:\n(.*?)(?:\n[A-Z][a-z]+:|\Z)', answer_raw, re.DOTALL)
    answer_clean = match.group(1).strip() if match else "No se encontró una respuesta útil."

    # ✅ Guardar solo la respuesta limpia en el historial
    st.session_state.chat_history.append((user_input, answer_clean))

    # ✅ Mostrar la respuesta limpia en la interfaz
    st.chat_message("user", avatar="✉").markdown(user_input)
    st.chat_message("assistant", avatar="🤖").markdown(answer_clean)
